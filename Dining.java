...
class Fork {
 ...
 public void draw(Graphics g) {
	 FORK_IMG = ImageUtills.loadImage("fork.png");
	 g.drawImage(FORK_IMG,x-XSIZE/2, y-YSIZE/2, XSIZE, YSIZE,null);
...
static Random random = new Random();
 ...
 public Philosopher(Table T, int cx, int cy,
                    Fork lf, Fork rf, Coordinator C, int philNum) {
     t = T;
     x = cx;
     y = cy;
     left_fork = lf;
     ...
     prn = new Random();
     color = THINK_COLOR;
     PHIL_IMG = ImageUtills.loadImage("phil_think.png");
     this.philNum = philNum;
//     THINK_TIME += prn.nextDouble()/10;
//     FUMBLE_TIME += prn.nextDouble()/10;
//     EAT_TIME += prn.nextDouble()/10;
 }
 ...
 public void draw(Graphics g) {
     ...
     //g.setColor(color);
     //g.fillOval(x-XSIZE/2, y-YSIZE/2, XSIZE, YSIZE);
	 g.drawImage( PHIL_IMG,x-XSIZE/2, y-YSIZE/2, XSIZE, YSIZE,null);
 }
 ...
 private void think() throws ResetException {
     double timeOffset = prn.nextDouble()/10;

     color = THINK_COLOR;
     PHIL_IMG = ImageUtills.loadImage("phil_think.png"); /* change to png */
     
     if(Dining.verbose) {
     System.out.println("Philosoper " + philNum + " thinking");}
     t.repaint();
     delay(THINK_TIME + timeOffset);
 }

 private void hunger() throws ResetException {
     color = WAIT_COLOR;
     PHIL_IMG = ImageUtills.loadImage("phil_wait.png");
     
     double timeOffset = prn.nextDouble()/10;
     t.repaint();
     if(Dining.verbose) {

     System.out.println("Philosoper " + philNum + " waiting");}
     //System.out.println(timeOffset);

     //System.out.println(FUMBLE_TIME);
     delay(FUMBLE_TIME + timeOffset);
     while(left_fork.inUse) {
    	 delay(FUMBLE_TIME + timeOffset);
     }
     left_fork.acquire(x, y);
     yield();    // you aren't allowed to remove this
     while(right_fork.inUse) {
    	 delay(FUMBLE_TIME + timeOffset);
     }
 ...
 private void eat() throws ResetException {
     ...
     color = EAT_COLOR;
     PHIL_IMG = ImageUtills.loadImage("phil_eat.png");
     
     t.repaint();
     if(Dining.verbose) {
     System.out.println("Philosoper " + philNum + " eating");}
     delay(EAT_TIME + timeOffset);
     left_fork.release();
     yield();    // you aren't allowed to remove this
     right_fork.release();
 }
